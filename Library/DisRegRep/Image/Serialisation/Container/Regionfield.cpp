...
void Protocol<Regionfield>::read(const Tiff& tif, Serialisable& regionfield) {
	...
	const auto bps_result = Bit::BitPerSampleResult(
		std::type_identity<Serialisable::ValueType> {}, tif.getField<std::uint16_t>(TIFFTAG_BITSPERSAMPLE).value());

	DRR_ASSERT(tif.isTiled());

	const auto rf_extent = static_cast<Serialisable::DimensionType>(tif.getImageExtent());
	regionfield.RegionCount = tif.getField<Serialisable::ValueType>(TiffTag::RegionCount).value();
	...
	for (const auto offset : Index::ForeachTile(rf_extent, tile_extent)) [[likely]] {
		...
		tile_matrix.toMatrix(rf_matrix, make_from_tuple<Serialisable::DimensionType>(offset));
	}
}
