...
void Protocol<Regionfield>::write(const Tiff& tif, const Serialisable& regionfield, const Tiff::ColourPaletteRandomEngineSeed seed) {
	using RegionfieldDimension3 = glm::vec<3U, Regionfield::IndexType>;
	...
	DRR_ASSERT(glm::all(glm::greaterThan(rf_extent, Regionfield::DimensionType(0U))));

	//The number of bits per sample must be a power of two, so we are taking the ceiling first.
	...
	tif.setImageExtent(RegionfieldDimension3(rf_extent, 1U));
	tif.setOptimalTileSize();

	Buffer::Tile<Regionfield::ValueType> tile_buffer;
	tile_buffer.allocate(tif.tileSize());
	const span raw_buffer = tile_buffer.buffer();

	const auto tile_extent = static_cast<Regionfield::DimensionType>(tif.getTileExtent());
	...
	for (const auto offset : Index::ForeachTile(rf_extent, tile_extent)) [[likely]] {
		...
		tile_matrix.fromMatrix(rf_matrix, make_from_tuple<Regionfield::DimensionType>(offset));
		...
		tif.writeTile(raw_buffer, RegionfieldDimension3(offset_y, offset_x, 0U), 0U);
	}
}